...
public class DuneChunkProvider implements IChunkProvider
{
    /** RNG. */
    private Random rand;
    ...
    {
        int i = par3ArrayOfByte.length / 256;
        ...
        for (int j = 0; j < 16; j++)
        {
            for (int k = 0; k < 16; k++)
            {
                for (int l = 0; l < i; l++)
                {
                    ...
                    else if (l <= (int)height)
                    {
                        i1 = Block.sand.blockID;
                    }

                    par3ArrayOfByte[j << 11 | k << 7 | l] = (byte)i1;
                }
            }
        }
    }
