...
public class NetClientHandler extends NetHandler
{
    ...
    public void handleVehicleSpawn(Packet23VehicleSpawn par1Packet23VehicleSpawn)
    {
        ...
        {
            Iterator var12 = ModLoader.getTrackers().values().iterator();

            while (var12.hasNext())
            {
                EntityTrackerNonliving var10 = (EntityTrackerNonliving)var12.next();
        ...
        if (var8 != null)
        {
            ...
            if (par1Packet23VehicleSpawn.throwerEntityId > 0)
            {
                ...
                {
                    Entity var15 = this.getEntityByID(par1Packet23VehicleSpawn.throwerEntityId);

                    if (var15 instanceof EntityLiving)
                    {
                        EntityArrow var16 = (EntityArrow)var8;
                        var16.shootingEntity = var15;
                    }
                }
    ...
    public void handleTileEntityData(Packet132TileEntityData par1Packet132TileEntityData)
    {
        if (this.mc.theWorld.blockExists(par1Packet132TileEntityData.xPosition, par1Packet132TileEntityData.yPosition, par1Packet132TileEntityData.zPosition))
        {
            ...
            if (var2 != null)
            {
                ...
                else if (par1Packet132TileEntityData.actionType == 255)
                {
                    if (par1Packet132TileEntityData.customParam1 == null)
                    {
                        return;
                    }

                    String var3 = par1Packet132TileEntityData.customParam1.getString("id");
                    String var4 = (String)TileEntity.func_85028_t().get(var2.getClass());
