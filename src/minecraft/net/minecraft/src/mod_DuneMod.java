...
public class mod_DuneMod extends BaseMod{
	...
	public void load() {
			...
			int sandWormID = ModLoader.getUniqueEntityId();
			int sandWormSegmentID = ModLoader.getUniqueEntityId();
			
	        ModLoader.addLocalization("generator.void", "Dune"); // Add dune world generator
	        ModLoader.registerEntityID(SandWormEntity.class, "SandWorm", sandWormID);
	        ModLoader.registerEntityID(SandWormSegment.class, "SandWormSegment", sandWormSegmentID);
	        
	        ModLoader.addSpawn(SandWormEntity.class, 1, 5, 10, EnumCreatureType.monster, new BiomeGenBase[] { new BiomeGenDune(23) });  //might not work right
